    ...
    for obj_dict in obj_list:
        try:
            if obj_dict["car_type"] == "car":
                car = Car(brand = obj_dict["brand"], photo_file_name = obj_dict["photo_file_name"], carrying = float(obj_dict["carrying"]), passenger_seats_count = int(obj_dict["passenger_seats_count"]))
                car_list.append(car)
            if obj_dict["car_type"] == "truck":
                truck = Truck(brand = obj_dict["brand"], photo_file_name = obj_dict["photo_file_name"], carrying = float(obj_dict["carrying"]), body_whl = obj_dict["body_whl"])
                car_list.append(truck)
            if obj_dict["car_type"] == "spec_machine":
                spec_machine = SpecMachine(brand = obj_dict["brand"], photo_file_name = obj_dict["photo_file_name"], carrying = float(obj_dict["carrying"]), extra = obj_dict["extra"])
